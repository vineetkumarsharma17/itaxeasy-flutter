                            ...
                            child: Material(
                              ...
                              child: InkWell(
                                onTap: ()async {
                                  ...
                                  if(nameCount.text.isEmpty||emailCount.text.isEmpty||passwordCount.text.isEmpty ||phoneCount.text.isEmpty||pinCode.text.isEmpty||lastName.text.isEmpty){
                                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Please Fill The Given Field"),));
                                  }if (!RegExp("^[a-zA-Z0-9+_.-]+@[a-zA-Z0-9.-]+.[a-z]").hasMatch(emailCount.text)) {
                                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Email Id should be valid"),));
                                  }
                                  if(passwordCount.text.length < 5){
                                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Password Should be more than 5 digits"),));
                                  }if(isChecked == false){
                                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Please Check the box"),));
                                  }if(phoneCount.text.length < 10){
                                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Number should be 10  digits!"),));
                                  }else{
                                    setState(() {
                                      isLoading = true;
                                    });
                                    ...
                                    final result = await  apiServices.register(insert);
                                    setState(() {
                                      isLoading = false;
                                    });
                                    if(result.resposeCode == 200){
                                      ...
                                      )).closed.then((value) => Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (context)=>const LoginPage()), (route) => false));
                                    }else{
                                     String  errorMessage=   await storage.read(key: "emailMessage");
                                      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          GestureDetector(
                            ...
                            child: Text('Login', style: regular16pt.copyWith(color: Colors.purple),),
                          ),
                        ],
                      ),
                    ],
