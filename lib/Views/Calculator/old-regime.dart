                        ...
                        children: [
                          ...
                          const Padding(
                            padding: EdgeInsets.only(
                                left: 10, bottom: 10, top: 10),
                            child: Align(alignment: Alignment.centerLeft,
                              child: Text("Interest paid on let out hp loan",
                                style: TextStyle(
                                  fontFamily: "Poppins",
                                  fontWeight: FontWeight.w500,
                                  letterSpacing: 1.5,
                      ...
                      child: GFButton(
                          onPressed: () async {
                            ...
                            insert = OldRegime(
                              ...
                              basicSalary: int.parse(basicSalaryCont.text),
                              hraReceived: int.parse(hraCont.text),
                              rentPaid: int.parse(rentCont.text),
                              address: address1,
                              otherAllowances: int.parse(otherACount.text),
                              interestPaidOnLetOutHpLoan: int.parse(
                                  letCount.text),
                              rentReceived: int.parse(rentRCount.text),
                              propertyTaxPaid: int.parse(propertyTaxCont.text),
                              interestPaidOnSelfOccupiedHpLoan: int.parse(
                                  selfCont.text),
                              savingsInterest: int.parse(savingCont.text),
                              fdInterest: int.parse(FDCount.text),
                              dividendIncome: int.parse(dividentCont.text),
                            ...
                            final result = await apiServices.gstOldRegime(insert);
                            ...
                            if(result.data != null){
                              Navigator.push(context, MaterialPageRoute(builder: (context)=> OldRegimeRsoo(
                                financialYear: financialCont.text,pan:panCont.text,filingCategory:fillingCont.text,residentialStatus:residentialCont.text,
                                basicSalary:int.parse(basicSalaryCont.text),hraReceived: int.parse(hraCont.text),
                                rentPaid:int.parse(rentCont.text),address:address1,otherAllowances:int.parse(otherACount.text),
                                interestPaidOnLetOutHpLoan:int.parse(
                                    letCount.text),rentReceived: int.parse(rentRCount.text),propertyTaxPaid:int.parse(propertyTaxCont.text),
                                interestPaidOnSelfOccupiedHpLoan: int.parse(
                                    selfCont.text),savingsInterest:int.parse(savingCont.text),fdInterest: int.parse(FDCount.text),
                                dividendIncome:int.parse(dividentCont.text),otherIncome:int.parse(OtherInCont.text),
                                elss:int.parse(elss.text),nps:int.parse(nps.text),ppf:int.parse(pff.text),lic:  int.parse(lic.text),
                              )));
                            }else{
                              print("error");
                            }
